...
Before any operation, resolve `$PROJECT`:

```bash
CONFIG_FILE=~/.config/card-carousel/repo_path
if [ -f "$CONFIG_FILE" ] && [ -d "$(cat "$CONFIG_FILE")/.git" ]; then
  ...
  PROJECT=""
  for candidate in \
    ~/Documents/RedCode/card-carousel \
    ~/Documents/card-carousel \
  ...
  if [ -z "$PROJECT" ]; then
    PROJECT=$(find ~ -maxdepth 4 -type d -name "card-carousel" -exec test -d "{}/.git" \; -print -quit 2>/dev/null)
  fi
  if [ -z "$PROJECT" ]; then
    echo "ERROR: card-carousel repo not found. Clone it first: git clone <repo-url>"
    exit 1
  fi
  mkdir -p ~/.config/card-carousel
  echo "$PROJECT" > "$CONFIG_FILE"
  echo "Cached card-carousel path: $PROJECT"
fi
...
- Repo cloned locally and discoverable via the Step 0 lookup above
...
> 人像卡片 — 真人照片背景 + AI插画 + 字幕
>
> **模板3 — dark-card**（1080×1920）
> 暗色卡片 — 白色卡片浮在黑底 + 分隔线 + 字幕底部
>
> **模板4 — sketch-card**（1080×1440）
> 手绘素描卡片 — 全屏铅笔插画 + 顶部标题栏 + 底部白色字幕
...
| `dark-card` | 1080x1920 (9:16) | 暗色卡片 — 白色卡片浮在黑底 + 分隔线 + 字幕底部 |
| `sketch-card` | 1080x1440 (3:4) | 手绘素描卡片 — 全屏铅笔插画 + 顶部标题栏 + 底部白色字幕 |

这些模板共享统一的模板注册与 defaults 合并机制；不同模板可有各自的画布尺寸、默认布局、封面实现和 positionable 元素集。
...
| 分辨率 | 1080x1920 (9:16) |
| 配色 | 暗底白卡片，金色强调 |
| 品牌 | 作者 `@黄赋`，slogan `关注我，陪你打怪升级` |
| 语音 | 火山引擎 `zh_male_ruyayichen_uranus_bigtts` |
| 生图 | Gemini 主 / Kling、Doubao 备 |
| 画风 | 黑白铅笔素描，漫画分镜感，允许中文手写标注 |

### sketch-card
| 配置项 | 默认值 |
|--------|--------|
| 分辨率 | 1080x1440 (3:4) |
| 配色 | 纯白背景，红色强调，底部白色字幕区 |
| 品牌 | 主题句 + 口号 + tagline |
| 语音 | 火山引擎 `zh_male_ruyayichen_uranus_bigtts` |
| 生图 | Gemini 主 / Doubao 备 |
| 画风 | 手绘素描信息图，白底，黑线条，少量红色点缀 |
...
4. Template `__init__.py` should expose:
   - `get_manim_script()` — 正文 scene.py 路径
   - `get_scene_classes()` — 正文 Scene 类名列表
   - `get_cover_manim_script()` — 封面脚本路径；无封面则返回 `None`
   - `get_cover_scene_class()` — 封面 Scene 类名；无封面则返回 `None`

5. Template scene.py must:
   ...
   - 若依赖插画或封面图，读取 `CARD_CAROUSEL_ILLUSTRATIONS_DIR` / `CARD_CAROUSEL_COVER_DIR`

6. New templates no longer need changes in `core/orchestrator.py` to wire cover rendering.
   Once the template exposes cover metadata through `BaseTemplate`, `step_cover` will discover it automatically.
...
pipeline.py (CLI entry)
  ...
  → core/orchestrator.py (keywords → tts → illustrations → render → voice → cover → concat)
      ...
      → templates/ (registry + BaseTemplate ABC)
          ...
          → dark_card/defaults.yaml
          → sketch_card/defaults.yaml
