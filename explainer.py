...
布局 (从上到下):
  ┌─────────────────────────────┐
  │ (深)logo  @深度进化Theo      │  ← 固定: 左上角圆圈logo + 作者名
  │                             │
  │ 在专业面前像学徒,在困难面前像王者│  ← 固定: 红色主题句(居中)
  │                             │
  │  面对专业，他们毫无           │  ← 切换: 大号黑色粗体(当前句子)
  │      敬畏                   │
  │                             │
  │   SHEN DU JIN HUA LUN      │  ← 固定: 拼音，浅灰
  │                             │
  │       🚀 [插画]             │  ← 切换: 居中插画(左右滑动)
  │                             │
  │                  个人观点    │  ← 固定: 右下角免责
...
config.background_color = "#FFFFFF"  # 白色背景
...
C_TEXT = "#000000"      # 纯黑文字
C_RED = "#C0392B"       # 红色主题句
C_PINYIN = "#000000"    # 拼音纯黑
C_MUTED = "#999999"     # 灰色辅助文字（仅免责声明）
C_BLACK = "#DBDADB"     # 底栏背景色（浅灰）
C_WHITE = "#1A1A1A"     # 底栏文字（深色）

# ── 路径 ──
_DIR = os.path.dirname(os.path.abspath(__file__))
AUDIO_DIR = os.path.join(_DIR, "media", "videos", "explainer", "audio")
TIMING_FILE = os.path.join(_DIR, "media", "videos", "explainer", "_timing.json")
...
def _build_pinyin(cfg):
    """构建拼音行（位置由 Scene 动态控制）"""
    brand_cfg = cfg.get("brand", {})
    pinyin = brand_cfg.get("pinyin", "")
    pinyin_text = Text(
        pinyin,
        font=FONT, font_size=16, color=C_PINYIN,
    )
    pinyin_text.move_to(UP * 1.8)
    return pinyin_text


def _build_disclaimer(cfg):
    """构建右下角免责声明"""
    brand_cfg = cfg.get("brand", {})
    disclaimer = brand_cfg.get("disclaimer", "个人观点\n仅供参考")
    text = Text(
        disclaimer,
        font=FONT, font_size=14, color=C_MUTED,
        line_spacing=1.2,
    )
...
def _build_footer_bar(cfg):
    ...
    bar = Rectangle(
        ...
        height=BAR_H,
        fill_color=C_BLACK, fill_opacity=1,
        stroke_width=0,
    )
    ...
    for line in tags_lines:
        t = Text(line, font=FONT, font_size=17, color=C_WHITE)
        tag_texts.add(t)
...
class Scene01_Cards(Scene):
    def construct(self):
        ...
        # ── 固定元素 ──
        logo_header = _build_logo_header(cfg)
        topic_line = _build_topic_line(cfg)
        pinyin = _build_pinyin(cfg)
        disclaimer = _build_disclaimer(cfg)
        footer_bar = _build_footer_bar(cfg)
        ...
        for i, sentence in enumerate(sentences):
            # 大号黑色粗体文字（每行≤9字换行，还原原视频排版）
            title = Text(
                _wrap_chinese(sentence, 9),
                font=FONT,
                font_size=44,
                color=C_TEXT,
                weight=BOLD,
