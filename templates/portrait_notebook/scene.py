...
# 注入项目根目录到 sys.path，确保 core.utils 可导入
if _DIR not in sys.path:
    sys.path.insert(0, _DIR)

# 导入坐标转换工具
from core.utils import percent_to_manim, get_element_position
...
def _get_font_size(cfg):
    return cfg.get("layout", {}).get("font_size", 32)
...
def _hex_to_rgb(color, fallback=(0, 0, 0)):
...
def _build_illustration(keyword, cfg, colors, font, illus_size):
    max_illus_w = 6.5
    max_illus_h = 4.8
    target_width = min(illus_size, max_illus_w)
    ...
    if not img_path:
        if not use_placeholder_only:
            return None
        placeholder_height = min(target_width * 0.75, max_illus_h)
    ...
    illus.scale_to_fit_width(target_width)
...
def _build_notebook(colors):
    """构建笔记本容器：米白圆角矩形 + 左侧螺旋装订线（返回 Group）

    使用 Group 而非 VGroup，因为后续需要添加 ImageMobject（插画），
    而 VGroup 只能包含 VMobject 子类。
    """
    notebook_width = 7.5
    notebook_height = 5.5
    ...
    # 用 Group 混合 VMobject（矩形/圆圈）和 ImageMobject（插画）
    return Group(notebook_rect, spiral_circles), notebook_rect
...
class PortraitNotebookScene(Scene):
    def construct(self):
        ...
        if len(topic) > 0:
            # 使用配置的位置，回退到默认布局（topic 是绝对定位元素）
            topic_mx, topic_my = get_element_position(
                cfg, "topic", config.frame_width, config.frame_height,
                fallback_fn=lambda: (0, 2.2)
            )
            topic.move_to([topic_mx, topic_my, 0])
            # 如果宽度超出屏幕，缩放适配
            if topic.width > 8.5:
        ...
        # ── 笔记本容器（初始放置在画面中部）──
        # illustration 是 flow_layout=true 元素；无覆盖时使用中心位置
        notebook_mx, notebook_my = get_element_position(
            cfg, "illustration", config.frame_width, config.frame_height,
            fallback_fn=lambda: (0, 0.0)
        )
        notebook_y = notebook_my

        # ── 固定元素：副标题（笔记本下方固定）──
        subtitle = _build_subtitle(cfg, colors, font)
        # subtitle 是 flow_layout=true 元素；无覆盖时动态跟随笔记本
        subtitle_mx, subtitle_my = get_element_position(
            cfg, "subtitle", config.frame_width, config.frame_height,
            fallback_fn=lambda: (0, notebook_y - 5.5 / 2 - 1.5)
        )
        subtitle_y = subtitle_my
        # 确保副标题不超出底部
        if subtitle_y < -7.2:
            subtitle_y = -7.2
        if len(subtitle) > 0:
            subtitle.move_to([subtitle_mx, subtitle_y, 0])
        self.add(subtitle)
        ...
        for i, sentence in enumerate(sentences):
            ...
            if prev_notebook_group is None or need_new_illus:
                ...
                notebook_group.move_to([notebook_mx, notebook_my, 0])

                # ── 插画（放在笔记本内部）──
                if need_new_illus:
                    illus = _build_illustration(kw, cfg, colors, font, illus_size)
                    if illus is not None:
                        # 插画（或占位图）居中放在笔记本矩形内（偏右以避开螺旋线）
                        illus.move_to([notebook_group.get_center()[0] + 0.2,
                                       notebook_my, 0])
                        notebook_group.add(illus)  # Group 支持 ImageMobject
            else:
